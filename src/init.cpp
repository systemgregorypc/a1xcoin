...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("a1xcoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  a1xcoind [options]                     " + "\n" +
                  "  a1xcoind [options] <command> [params]  " + _("Send command to server or paycoind") + "\n" +
                  "  a1xcoind [options] help                " + _("List commands") + "\n" +
                  "  a1xcoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "a1xcoin:"))
                fCommandLine = true;
...
{
    ThreadSafeMessageBox(str, _("a1xcoin"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>          "   + _("Specify configuration file (default: a1xcoin.conf)") + "\n" +
        "  -pid=<file>           "   + _("Specify pid file (default: a1xcoind.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-reservebalance")) // a1xcoin: reserve balance amount
    {
    ...
    // Make sure only a single a1xcoin process is using the data directory.
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  a1xcoin is probably already running."), pszDataDir));
    ...
    //
    if (fDaemon)
        fprintf(stdout, "a1xcoin server starting\n");
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("a1xcoin"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of a1xcoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart a1xcoin to complete") << "\n";
